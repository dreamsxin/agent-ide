...
- **Stop refuses side-effecting calls that have not started yet.** The run's cancel flag is the same `Arc<AtomicBool>` the tool surface holds (`WorkspaceToolPermissions::fresh_cancel()` mints it, `try_begin_run` publishes it), so `WorkspaceToolInvoker::invoke()` refuses `workspace_run_command`, the write / delete / move tools and both browser tools once Stop has been pressed. Read-only tools still run — they change nothing, and refusing them would only add noise to a transcript that is being abandoned. A browser attempt refused this way is recorded as `browser_open_cancelled` / `browser_tabs_cancelled` and counted as *not performed*. What this does **not** do: it does not interrupt a call that is already executing (a spawned command runs to completion; an in-flight CDP request is bounded only by the timeout below), and it does not cover MCP tools, which have no permissions object and no switch. The tool loop checks the same flag between calls, so the gate is defence in depth for the built-in tools, not a kill switch for the OS.
