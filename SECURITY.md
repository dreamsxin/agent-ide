...
- **The window must actually be in the foreground, not merely asked to be.** `SetForegroundWindow` is asynchronous: a non-zero return means the request was accepted, not that the switch has happened. The click is sent only after `GetForegroundWindow()` confirms that window, polled for up to 500 ms; if it never gets there the click is refused. Without the confirmation the guarantee would be empty — `SendInput` delivers to screen coordinates, so the click would land on whatever is still on top.
- **The window is measured *after* activation.** Activating a window can change its rect (a minimized window is restored at that moment), so the size check compares against the post-activation rect.
- **A resized window is refused.** If the rect no longer matches the frame, the coordinates no longer point at the same thing; "close enough" here means clicking something random.
...
Known limits: the click is real system input, so it lands wherever that window is — if the user moves the mouse or types at the same moment, the two streams interleave. Stop does not abort a click already in flight (it is a single Win32 call, so the window is microseconds, not seconds), but the foreground wait before it can take up to 500 ms. Coordinates come from `GetWindowRect` and are sent through the virtual-screen metrics, so both sides share whatever DPI view the process has; the pairing has not been exercised on a mixed-scaling multi-monitor setup, and the frame's size check is the thing that would catch a mismatch rather than silently mis-clicking. Clicking is **not** in the preset ladder: all three presets set `allowComputerInput: false` with an empty list.
