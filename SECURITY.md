...
- **A scroll is bounded to 10 notches and 0 is refused.** Approval is per call, so an unbounded `notches` would turn one approval into unlimited authority; and reporting success for a scroll that moved nothing would tell the model the page is at its end. Both are rejected while the arguments are being parsed, before the frame is looked up, so neither reaches the approval prompt nor leaves a record.
...
- **The whole gesture goes in a single `SendInput` call** — move plus down/up, both down/up pairs of a double click, or move plus wheel — so a user moving the real mouse mid-sequence cannot turn a click into a drag or split a double click into two singles. If only part of the batch is accepted, the buttons *that gesture pressed* are released so the desktop is not left mid-drag (a scroll presses none, and gets no spurious mouse-up), and the call still reports failure.
- **The cursor is put back.** Its position is read before the input and restored after, so the pointer does not stay parked where the Agent aimed, silently changing hover state and the user's next real click.
- **Every attempt is recorded** as `computer_click` or `computer_scroll` with the usual `_refused` / `_cancelled` / `_failed` suffixes, and *every* one of them carries what was attempted and where: gesture, coordinates, window title, frame size. A record that says only "the user denied this action" against an app name cannot be reconstructed later, which defeats the point of keeping it. Refusals after Stop are recorded against `desktop`, not `chrome` — that default previously mislabelled a refused click as a browser action.
...
- **The injection has a test that really clicks — when the session allows it.** `a_click_reaches_the_window_it_was_aimed_at` creates a real window and calls the production path: if the foreground switch is granted it asserts the window received the click inside its client area; if the session refuses (a `cargo test` run launched from a background shell, or a headless CI session) it asserts that the click was refused and nothing was sent, and prints which path it took. So the suite covers the injection *conditionally* — the printed line is the only honest way to know whether a given run exercised `SendInput`. The event sequence each gesture produces is pinned separately by pure-function tests (`each_gesture_sends_its_own_buttons`, `scrolling_is_measured_in_wheel_deltas`), because "the right button, the right number of times" is otherwise only observable by watching the screen.
