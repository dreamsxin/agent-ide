...
- **Stop refuses side effects that have already reached the tool layer.** The run's cancel flag is the same `Arc<AtomicBool>` the tool surface holds (`WorkspaceToolPermissions::fresh_cancel()` mints it, `try_begin_run` publishes it), so `invoke()` refuses `workspace_run_command`, the write / delete / move tools and both browser tools once Stop has been pressed. Previously cancellation only took effect *between* tool calls: a call already inside the tool ran the command or opened the page anyway, while the UI showed Idle. Read-only tools still run — they change nothing, and refusing them would only add noise to a transcript that is being abandoned. A browser attempt refused this way is recorded as `browser_open_cancelled` / `browser_tabs_cancelled`. The inherited tool surface of the auto-repair loop is the exception; see the limitations below.
...
- **A run that only navigated does not badge the Changes tab.** The tab count is derived from pending diffs, so the user has to open the tab to see the actions. The action-log entry is the notification path today.
...
- **A continued pipeline uses the authority captured at its start.** `continue_agent_pipeline` clones the permissions stored on the orchestrator, so revoking Browser Use in Settings does not affect a paused run that is then continued. The same is true of command execution. (It does mint a fresh side-effect switch, so Stop still applies to the continuation.)
- **The auto-repair loop's side-effect gate is not wired.** `repair_workspace` does not build its own tool surface — `drive_repair` reuses the invoker left by the previous run, whose switch belongs to that run. Stop still stops the repair loop's model calls, and the repair run's writes and external actions are now drained and recorded, but a tool call already inside `invoke()` on that path is not refused. Wiring it means rebuilding the tool surface for the repair run; tracked in ROADMAP Known Issues 59.
