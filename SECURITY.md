...
- **The durable log is itself a disclosure surface.** It contains the same strings the review area shows: full URLs including query strings (`browser_open`, `browser_read_page`) and window titles (`computer_capture`). SECURITY.md already names magic links and OAuth callbacks with tokens in the query string as a real exposure for `workspace_browser_tabs`; those now persist in plaintext in the home directory, with no age bound, no redaction, and no in-app way to prune or clear them. There is no file-permission hardening on `~/.agent-ide`. That is a deliberate trade — an audit record that forgets is not an audit record — but it is a trade, and a "clear the external action log" control is the open item.
