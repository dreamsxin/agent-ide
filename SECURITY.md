...
- **Each gesture has a test that really sends it — when the session allows it.** `a_click_reaches_the_window_it_was_aimed_at`, `a_right_click_arrives_as_a_right_click`, `a_double_click_arrives_as_one_double_click` and `a_scroll_arrives_with_the_notches_it_asked_for` create a real window and call the production path. If the foreground switch is granted they assert the window received *that* input and not another: the click also checks the point is inside the client area, the right click asserts no left click arrived, the double click asserts one `WM_LBUTTONDBLCLK` rather than two singles, and the scroll asserts the delta is `-2 × 120` — direction included. If the session refuses (a `cargo test` run launched from a background shell, or a headless CI session) each asserts the call was **refused** and that *no* input of any kind arrived, and prints which path it took. So the suite covers the injection *conditionally*, and the printed line is the only honest way to know whether a given run reached `SendInput` — on a background-shell run, all four take the refusal branch.
- **The counters those tests depend on are verified unconditionally.** Because the arrival branch does not run in every session, a mis-decoded wheel delta or a double click recorded as a left click would leave those tests green forever. `the_test_window_counts_each_kind_of_mouse_input_separately` posts the four messages straight to the window and checks each counter and the signed delta, so the evidence the injection tests rely on is itself tested. The event sequences are pinned separately by pure-function tests (`each_gesture_sends_its_own_buttons`, `scrolling_is_measured_in_wheel_deltas`, `only_the_buttons_left_held_by_the_accepted_part_are_released`).
