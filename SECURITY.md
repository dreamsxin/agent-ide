...
- **The approved window is re-identified before the pixels are read.** An `HWND` cannot cross an `await`, so the enumeration runs again after approval; `capture_approved_target` refuses unless the newly matched window has the same title and app. Without that check, approving "Docs — pricing" and capturing "Signal — Alice" would be reachable by nothing more than a focus change. A resize is not treated as a different window, but the pixel cap is re-checked against the new size.
- **Windows outside the allowlist are not even named** in the refusal — it reports only how many allowed windows exist.
- **Two size gates, in this order**: the pixel count (4 000 000, so a 2560×1440 window fits) is checked before any bitmap is copied — and before the user is asked, so a window that would be refused anyway never produces a prompt — and the encoded PNG is checked against the same 4 MiB per-image cap `workspace_read_image` uses. It also charges the same 16 MiB per-run and 12 MiB per-request image budgets, so a capture and a file read compete for one allowance.
