...
- Its tool rounds are capped at 8 and the cap is enforced. When it stops a child early, the caller is told — and the two facts behind that sentence (`tool_rounds`, `hit_round_cap`) come from the loop's own bookkeeping rather than from counting messages afterwards, because the last round executes no tools and history trimming deletes the oldest ones. A half-finished answer that reads as finished is worse than none.
...
- **Nothing about what a tool actually does.** Arguments are forwarded opaquely. There is no path validation, no workspace-boundary check, no read/write distinction, and the Agent-write deny list below does not apply. An MCP filesystem tool can write `.git/hooks/pre-commit` or read `~/.ssh/id_rsa`, and those effects never appear in the diff-review UI. What *is* enforced: after Stop, `McpToolInvoker::invoke` refuses to issue further calls and logs the refusal; a call already in flight cannot be cancelled.
- **Every call is recorded where it survives the window closing.** Each MCP call lands in the durable external-action log — `mcp_tool_call`, `mcp_tool_failed`, `mcp_tool_cancelled` — beside browser navigations and `web_fetch`, because it belongs to the same class: done, cannot be taken back. A failed call is recorded too: the error comes from the server, and is not evidence that nothing was written. Before this, an MCP call existed only in the in-memory action log, so "what did that server do yesterday" had no answer at all.
- **The record names the paths the arguments named**, split into inside and outside the workspace. Paths are read from a fixed list of argument keys (`path`, `file_path`, `files`, `cwd`, …) rather than sniffed out of every string: MCP arguments carry URLs, regexes and whole files, and guessing produces a log nobody reads. The classification is purely lexical — `..` is reduced, a sibling directory whose name shares a prefix is *outside* — and it never refuses the call. A key table cannot be an authority (the schema belongs to the server; rename a key and the check is gone), and refusing on a guess would break the filesystem servers a user deliberately pointed at another directory. So the boundary here is a *statement*, not a gate.
- **In-workspace files the arguments named are snapshotted before the call and compared after it.** Anything that really changed is published to the review area as an `applied` diff carrying its pre-call content, with an undo checkpoint — the same mechanism the built-in write tools use, except the "before" copy is taken *for* the tool instead of *by* it, because an MCP result is free text and never says what it wrote. Creations and deletions are detected too. Each candidate path goes through **`resolve_for_agent_write`** first, the same resolution a built-in write uses, so the deny list (`.git/`, `.env`, credential files) and symlinks leading out of the workspace are skipped rather than read into a review card and an undo snapshot — taking a copy means putting those bytes on screen and writing them back on undo.
- **The record says "changed while that tool ran", not "written by it".** The comparison establishes that the bytes differ across the call, not who changed them: a user saving that file in the editor during a long call, or a watcher rewriting a generated file, looks identical. The provenance rationale says so, because a user acting on a wrong cause can undo the wrong change. If one file is touched by both a built-in write tool and an MCP call, it produces **one** card and **one** undo checkpoint whose pre-image is the earliest of the two, and the rationale names both.
- **What that coverage does *not* include**, stated in the record itself: paths outside the workspace, deny-listed paths, files over 1 MiB, files that are not valid UTF-8, and any file the arguments never mentioned. Those changes remain invisible and cannot be undone from here. The 1 MiB bound is about latency — the file is read twice per call — not memory.
...
- **The shell differs by platform and the command is translated, not guessed at.** Windows runs `cmd /C`, everything else `sh -lc`. Before a command is executed, `command_text::portable_command` rewrites the POSIX-isms that `cmd` cannot express but that mean something mechanical: leading `VAR=value` (quoted or not) becomes `set "VAR=value" && …`, single-quoted arguments become double-quoted (single quotes are not quoting in `cmd`, so `'src/a b.ts'` would arrive as two arguments), and a top-level `;` becomes `&`. Escaped quotes inside double quotes are tracked, because miscounting one would hide a top-level `;` and silently drop the second half of the command. What has no `cmd` equivalent — `$(…)`, backticks, mixed quoting — and what *would change meaning* if converted — a quoted `%VAR%` (cmd expands inside double quotes) or an argument ending in a backslash (cmd reads it as escaping the quote) — is **refused with the reason**, because a command that succeeds with the wrong value is worse than one that fails, and a confusing shell error costs the model a whole round to diagnose. The tool description also states the platform, the shell and the four rules that actually differ; the record and the UI keep the command as it was written, so a run log never disagrees with the configured task. Nothing is rewritten on POSIX.
- **Child output is decoded by encoding, per line, not assumed to be UTF-8.** UTF-8 is tried first (modern toolchains emit it even on Windows), and only for the lines where that fails does it fall back to the Windows active code page (`GetACP`, mapped for CP936/54936/950/932/949/125x/866/874), with `AGENT_IDE_OUTPUT_ENCODING` as an override. Per line, because one pipe can carry both: `cargo build` on a Chinese Windows machine interleaves cargo's UTF-8 diagnostics with MSVC's CP936 lines, and judging the whole buffer would re-decode the correct half as GBK. Before this, `String::from_utf8_lossy` turned every CP936 byte into U+FFFD *before* the Problems panel parsed it, before the repair prompt quoted it, and before the model read it — so on such a machine a failing build's message was destroyed on the way in. An unknown code page still falls back to lossy rather than guessing a single-byte encoding, because a wrong guess produces plausible-looking text that nobody can spot.
...
1. **MCP tools are unconstrained.** They bypass the workspace boundary and the Agent write deny list entirely. Adding an MCP server is equivalent to granting arbitrary code execution. This is the single largest gap. Narrowed, not closed: every call is recorded in the durable external-action log, and a change to an in-workspace file the arguments named now reaches the review area as an applied diff with an undo checkpoint. Outside the workspace, over 1 MiB, non-UTF-8, or never named in the arguments — still invisible and still not undoable.
