...
- **The record is an emitted event, not persisted state.** `publish_external_actions` emits one action-log entry; the frontend keeps it in the log panel (bounded, per workspace, clearable). Unlike a `FileDiff` it is not stored in the backend and does not survive a restart, and `take_external_actions` clears the in-memory list when it publishes. This is weaker than what an irreversible capability deserves and is tracked in ROADMAP Known Issues 56.
