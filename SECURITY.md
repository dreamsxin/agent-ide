...
- **The approved window is identified by its handle, not by its title.** `resolve_capture_target` keeps the `HWND`'s *value* (an `isize`, which crosses an `await` fine) and `capture_approved_window` reads that handle back. Before the pixels are copied it re-reads the window (`computer::describe_window`) and refuses if it has closed, or if the handle now belongs to a different app — Win32 recycles handles, so "still valid" is not "still the same". Re-finding the window *by title* would be worse than nothing here: a window title is set by the thing being disclosed (a page's `document.title` is its window title), so the model can arrange for a second window to answer to the approved name.
...
A second, about verification rather than behaviour: **the capture approval path has no automated test.** `allows_capture()` requires `cfg!(windows)`, and on Windows `resolve_capture_target` enumerates the real desktop, so the tool cannot be driven deterministically in CI. What is covered is the decision it delegates to — `verify_approved_window` (closed window, recycled handle, changed title) and `CaptureTarget::describe` — plus the whole approval mechanism through `workspace_browser_open`, which is tested end to end. The wiring between them is checked by review only. That is weaker than the browser path and is not presented as equivalent.
...
- **`resolve_agent_approval` returns whether anyone was still waiting.** A click that arrives after the timeout returns `false`. What the UI does with that today is only a console warning — the dialog has normally already been closed by `agent-approval-closed`, so reaching this state requires that event to have been missed. Stated as it is rather than as a user-visible guarantee.
...
Scope today: `workspace_browser_open` and `workspace_computer_capture`. Headless entry points (the CLI) install no channel, so an irreversible action there is refused rather than performed unattended — which is currently moot, since the CLI grants neither browser nor desktop authority. Known gap: if the frontend cannot read a request (an `opType` a newer backend introduced), it logs and drops it, and the run waits out the full timeout before refusing. `tests/ipc-contract.test.ts` fails if the backend asks approval for an op type the frontend's normalizer does not accept, which is the cheap half of that problem.

Deliberately *not* behind a prompt: `workspace_browser_tabs` and `workspace_computer_windows`. Both disclose, but they disclose a *list* whose scope the allowlist already fixes, and both are called far more often than the two above — a prompt per call would be the kind of friction that trains people to click Approve without reading, which would cost more than it buys.
