...
- **Identity is re-verified with the same three checks as capture** (`ApprovedWindow::verify` — alive, same app, same pid), because a handle can be recycled to another window between the capture and the click. That catches recycling *across* processes; a window of the same app in the same process can still inherit the handle, and then only the frame's size check stands between the model and a sibling window it never captured. That residual is real and unfixed — Win32 offers no stable per-window identity.
...
- **Move, down and up go in a single `SendInput` call**, so a user moving the real mouse mid-sequence cannot turn the click into a drag. If only part of the batch is accepted, a compensating mouse-up is sent so the desktop is not left mid-drag, and the call still reports failure.
- **The cursor is put back.** Its position is read before the click and restored after, so the pointer does not stay parked where the Agent aimed, silently changing hover state and the user's next real click.
- **Every attempt is recorded** as `computer_click` / `_refused` / `_cancelled` / `_failed`, and *every* one of them carries where it was aimed: coordinates, window title, frame size. A record that says only "the user denied this action" against an app name cannot be reconstructed later, which defeats the point of keeping it.
- **The injection has a test that really clicks — when the session allows it.** `a_click_reaches_the_window_it_was_aimed_at` creates a real window and calls the production path: if the foreground switch is granted it asserts the window received the click inside its client area; if the session refuses (a `cargo test` run launched from a background shell, or a headless CI session) it asserts that the click was refused and nothing was sent, and prints which path it took. So the suite covers the injection *conditionally* — the printed line is the only honest way to know whether a given run exercised `SendInput`.
