...
- **The shell differs by platform and the command is translated, not guessed at.** Windows runs `cmd /C`, everything else `sh -lc`. Before a command is executed, `command_text::portable_command` rewrites the POSIX-isms that `cmd` cannot express but that mean something mechanical: leading `VAR=value` becomes `set "VAR=value" && …`, single-quoted arguments become double-quoted (single quotes are not quoting in `cmd`, so `'src/a b.ts'` would arrive as two arguments), and a top-level `;` becomes `&`. What has no `cmd` equivalent — `$(…)`, backticks, mixed quoting — is **refused with the reason**, because a confusing shell error costs the model a whole round to diagnose. The tool description also states the platform, the shell and the four rules that actually differ; the record and the UI keep the command as it was written, so a run log never disagrees with the configured task. Nothing is rewritten on POSIX.
- **Child output is decoded by encoding, not assumed to be UTF-8.** UTF-8 is tried first (modern toolchains emit it even on Windows), and only on failure does it fall back to the Windows active code page (`GetACP`, mapped for CP936/54936/950/932/949/125x/866/874), with `AGENT_IDE_OUTPUT_ENCODING` as an override. Before this, `String::from_utf8_lossy` turned every CP936 byte into U+FFFD *before* the Problems panel parsed it, before the repair prompt quoted it, and before the model read it — so on a Chinese Windows machine a failing build's message was destroyed on the way in. An unknown code page still falls back to lossy rather than guessing a single-byte encoding, because a wrong guess produces plausible-looking text that nobody can spot.
