...
- **The record names the paths the arguments named**, split into inside and outside the workspace, and says plainly that anything written is not in the review area and cannot be undone from here. Paths are read from a fixed list of argument keys (`path`, `file_path`, `files`, `cwd`, …) rather than sniffed out of every string: MCP arguments carry URLs, regexes and whole files, and guessing produces a log nobody reads. The classification is purely lexical — `..` is reduced, a sibling directory whose name shares a prefix is *outside* — and it never refuses the call. A key table cannot be an authority (the schema belongs to the server; rename a key and the check is gone), and refusing on a guess would break the filesystem servers a user deliberately pointed at another directory. So the boundary here is a *statement*, not a gate. What it buys: an MCP write outside the workspace is no longer invisible.
...
1. **MCP tools are unconstrained.** They bypass the workspace boundary, the Agent write deny list, and the diff-review UI entirely. Adding an MCP server is equivalent to granting arbitrary code execution. This is the single largest gap. Narrowed, not closed: every call now lands in the durable external-action log naming the tool and the paths its arguments mentioned (inside vs. outside the workspace), so the effects are *auditable afterwards* — but they are still not shown as diffs and still cannot be undone.
