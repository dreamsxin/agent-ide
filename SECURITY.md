...
- **`workspace_move_file` needs both `allow_write` and `allow_create`, and refuses to overwrite.** Moving makes a file disappear from one path (a write) and appear at a path that did not exist (a create), so it is advertised only when both grants are present and the invoker does not claim it otherwise. **Both** paths resolve through `resolve_for_agent_write`, so a credential file or anything under `.git/` can be neither the source nor the destination. An existing destination is an error, never an overwrite — overwriting would destroy content that no record could restore. Directories are refused, as with deletion. The move itself is a single `fs::rename`, which has two consequences worth stating: there is no half-done state where the file exists in both places, and the tool never reads the content, so binary files move as well as text. Undo is a move back rather than a content rewrite, so it restores the file itself and not just its bytes; if the file was edited after the move, undo also writes back the content as of the move.
