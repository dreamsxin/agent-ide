...
/** 行首那个字母是符号，不是文案：三档共用一列 24px，翻译过去要么放不下，要么三档一样宽 */
  ...
  return (
    <div data-testid="problems-panel" className="flex h-full flex-col bg-black text-xs">
      <div className="flex items-center gap-3 border-b border-surface-border px-3 py-1.5">
        <span className="font-semibold text-surface-text">{t("problems.title")}</span>
        <span className="text-diff-remove">{t("problems.errors", { count: counts.error })}</span>
        <span className="text-diff-modify">{t("problems.warnings", { count: counts.warning })}</span>
        <span className="text-accent-blue">{t("problems.info", { count: counts.info })}</span>
            ...
            return (
              ...
              >
                ...
                <span className="truncate text-[10px] uppercase text-surface-muted">
                  {problem.source} &middot; {t(`severity.${problem.severity}`)}
                </span>
                <span className="flex items-center gap-1">
                  <button
                    ...
                    title={t("problems.explain.title")}
                  >
                    {t("problems.explain")}
                  </button>
                  <button
