...
| Workspace read | `workspace_read_file`, `workspace_search_text`, `workspace_glob`, `workspace_grep`, `workspace_list_files`, `workspace_read_image` | Read-only. Path-taking tools resolve through `workspace::resolve_existing`; the search tools walk from the workspace root and skip symlinks. Credential files refused, 64 KB read cap, 60 search hits, 200 listed entries, 2 MB per searched file, 20 000 files per walk (reported when hit). |
