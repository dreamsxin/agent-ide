...
21. **Self-audit of the lock refactor found a real bug in the guard it introduced (fixed 2026-09-09)**
   ...
   - **Found in the same audit, not fixed here, in rough order of severity:**
     - `state_machine.rs` `(Done, UserPrompt) => Idle` resets to `Idle` and nothing re-fires `UserPrompt`, so `PlanReady` (needs `Thinking`) and `StepStart` (needs `Planning`) both fall through the catch-all — **a whole run reports `idle`**. `Done` is a normal resting state (clean Auto run, apply-all, any review refresh with no open diffs), and `TaskView` enables Run whenever the state is idle/done/waiting/error. So *apply all → new prompt → Run a step* enables single-step execution during a live pipeline with no race window at all. Pre-existing, and it is what makes the guard above actually reachable from the UI.
     - The `cancel_flag` is shared and every entry point resets it to `false` before starting, so a new run **un-cancels a still-draining old one**. Combined with the above, a stopped run can resume calling the model.
     ...
     - Frontend: entering focus mode and then clicking a status-bar segment leaves `focusMode: true` with a panel open, and the highlighted Focus button then *opens* all three panels; the incoherent pair is persisted. Pre-existing (the TopBar toggles and `Ctrl+Shift+E/X/\`` have the same hole) but the status bar added the entry point a user is most likely to hit while focused.
     - Frontend, mine: the problems segment encodes severity by colour only — no letters, no icons — so a sighted user with a colour-vision deficiency sees three bare integers. The component's own comment claims an "E/W/I convention" the code does not implement, which is the documentation-accuracy rule broken on its own line. The usage tooltip carrying the "this is a lower bound" warning is `title`-only, so keyboard and AT users never see it. The branch button has no `aria-label` and its action lives only in `title`.
