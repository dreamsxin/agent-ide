...
Status as of 2026-09-08: **Phase 8 daily IDE replacement hardening in progress; Phase 9.0 market-parity foundation nearly closed.** Landed since the 09-05 snapshot: priority-quota context budget, CJK-aware token estimate, tool surface on resumed runs, the write and verification tools, CLI tool parity, the bounded repair loop with its UI, and the per-run monetary spend cap (9.0.13, config-file only — no Settings field yet). Still open in 9.0: the persistent run message thread (9.0.11, interim size bound only).
...
| 9.0.13 Per-Run Spend Cap | Monetary cap alongside the existing `maxRunTokens` token cap; closes the last open item in 9.0.4 | Medium | **Done (2026-09-08), config-file only**: `RunUsageMeter` carries a `TokenPricing` (micro-USD per million tokens; integer arithmetic with ceiling rounding so sub-cent costs can't be rounded away into "free") and a `maxRunSpendMicros` limit. `check_budget` tests spend **before** tokens, so a run that is cheap in tokens but expensive in money still stops. Read from the profile by `llm_profiles::run_spend_cap`, applied in `AgentGlobalState::get_llm_client`. Deliberate design choice: pricing requires **both** halves (`promptMicrosPerMillion` + `completionMicrosPerMillion`) — with only one the estimate systematically undercounts, and an undercounting cap is worse than no cap because the user thinks they are protected. With pricing missing, spend is reported as `not computable (no pricing configured)` and the cap is not enforced, never as `$0.00`. **Not in the Settings UI**: the three fields must be edited in the profiles JSON by hand |
