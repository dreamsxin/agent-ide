...
| 9.0.4 Desktop Permission Model V2 | Ask/Suggest/Auto presets plus granular file/command/git toggles, path deny rules, and per-run cost caps, all enforced in the backend | High | **In progress**: MCP tool approval (`McpToolPolicy` + per-server `autoApprove`), `allowFileCreate` for Auto-mode apply, and path deny rules (`workspace::resolve_for_agent_write`) are enforced backend-side. `allowFileDelete` / `allowCommandRun` / `allowGitActions` have no Agent-reachable backend path today (see Current State). Per-run cost caps landed in 9.0.13 (token cap plus a monetary cap checked ahead of it) |
...
5. Finish Phase 9.0.4 desktop permission model V2. Done so far: MCP tool approval (`McpToolPolicy` + per-server `autoApprove`), `allowFileCreate` for Auto-mode apply, and path deny rules for Agent writes, all enforced in `src-tauri`. Still open: a per-run cost cap and exposing MCP tools to the CLI behind the same model. `allowFileDelete` / `allowCommandRun` / `allowGitActions` need an Agent-reachable backend path before they can be enforced at all — 9.0.8/9.0.9 create that path.
