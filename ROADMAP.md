...
- Documented the consequence in `docs/testing_strategy.md`: this E2E is a **manually triggered check on an idle machine**, not a regression net. It cannot run in CI (no interactive desktop) and cannot run on a machine in use (it takes over the foreground, mouse and clipboard). The automated net for backend behaviour is `agent_cli`. Also recorded the gap that matters most: the E2E profile uses `toolCallMode = "text_protocol"` and `stream_mock_chat` returns a plain `String`, so the mock provider cannot emit `tool_calls` — meaning neither the E2E nor the CLI smoke exercises the native tool loop, and `workspace_run_command` / `workspace_write_file` still have no automated end-to-end coverage.
...
| 9.0.12 CLI Tool Parity | `agent_cli` currently passes no invoker, so headless Agent runs are blind. Expose the same tool surface under the CLI permission flags | High | Planned |
