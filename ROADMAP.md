...
- **The new probe immediately found a real bug in the Git panel backend**: in a freshly `git init`-ed repository with no commits, `commands::git::git_status` returns `Err("HEAD: reference 'refs/heads/master' not found ... UnbornBranch")` because it calls `repo.head()` unconditionally. So the desktop Git panel shows an error instead of "no commits yet" for a brand-new project — a common first-run state. The probe classifies it as `unavailable` for now so the tool is usable; **the underlying `git_status` / `git_diff` behaviour is not fixed** and needs a decision about what `branch` and `entries` mean with no HEAD.
...
6. **Git workflow needs continued polish**
   ...
   - **Broken in a repository with no commits**: `git_status` and `git_diff` call `repo.head()` unconditionally, so a freshly `git init`-ed project returns `Err(... UnbornBranch)` and the panel shows an error instead of "no commits yet". Found by `agent_cli smoke ide-surface`. Needs a decision on what `branch` and `entries` mean with no HEAD before fixing.
...
17. Fix `git_status` / `git_diff` for repositories with no commits (see Known Issues 6). A first-run project currently gets an error panel instead of an empty-but-valid Git view.
