...
- Wired the per-run cap up: `maxRunTokens` on the LLM profile, read via `llm_profiles::run_token_cap`, and `AgentGlobalState::get_llm_client` now hands back a fresh `RunUsageMeter` alongside the client. Because the meter is attached where the client is built, every run command is metered and the cap applies without each command opting in. A configured `0` is treated as unset so a hand-edited config cannot lock every run out. Run end writes a `run_token_usage` action log that says "not reported by provider" rather than printing 0 when usage is unknown. Two known gaps: `continue_agent_pipeline` restarts accounting when it resumes a paused run, so the continuation does not count against the earlier segment's allowance, and there is no UI field for `maxRunTokens` yet — it has to be set in `config.json`.
