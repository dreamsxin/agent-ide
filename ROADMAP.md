...
| 9.0.12 CLI Tool Parity | `agent_cli` currently passes no invoker, so headless Agent runs are blind. Expose the same tool surface under the CLI permission flags | High | **Done (2026-09-07)**: `--allow-run` now switches the CLI to native tool calling and attaches `workspace_run_command` limited to those patterns. Without it nothing changes — no invoker, `text_protocol`, so arbitrary OpenAI-compatible endpoints never receive a `tools` parameter. `workspace_write_file` is deliberately **not** exposed: `--allow-edit` / `--allow-create` govern whether produced diffs may be applied, and reinterpreting them as "the model may write files" would be a silent privilege escalation |
