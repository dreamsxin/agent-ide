...
- Finished the migration in the same pass: `run` and `continue_pipeline_from` take `Arc<dyn RunEvents>` (owned, because they spawn token-forwarding tasks), and **`agent/orchestrator.rs` now imports no Tauri types at all**. The 2600-line file holding the pipeline no longer needs a desktop runtime to execute; what is missing is tests that drive it, not the ability to write them. Call-site churn was two lines — `Arc::new(app_handle.clone())` — because `AppHandle` implements the trait, so `&AppHandle` coerces at every other call site.
