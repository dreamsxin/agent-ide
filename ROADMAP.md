...
| 9.0.10 Autonomous Bounded Repair Loop | The prompt builder and check runner are already shared (`services/verification.rs`, `verify_workspace`, `agent_repair_prompt`), and `Verify All` / `Fix with Agent` send a repair prompt — but each is one user-triggered round. Missing: the orchestrator itself running verify → repair → re-verify within a bounded iteration count, so a failed stage or failed check does not abort the pipeline | Critical | **Backend done (2026-09-07), not reachable from the UI.** `AgentOrchestrator::repair_until_checks_pass` runs checks → repair prompt → apply → re-check until the checks pass or the budget is spent, sharing `verification::RepairPolicy` with the CLI so both entry points agree on when to give up. Exposed as the `repair_workspace` command, gated on Auto mode: every iteration must land on disk or the re-run tests the old code, and `suggest` / `edit` promise the opposite. Fixes go through `apply_all_diffs`, so each round stays undoable. Two tests drive it headlessly — one asserts the model is never called when the checks already pass, the other that a hopeless run stops at its budget and records every iteration. **UI wired 2026-09-08**: `Auto Repair` in the Commands panel (`TasksPanel.tsx`) calls it with the discovered checks and a 2-round budget, and reports the round count plus the stop reason. Three vitest cases cover the call payload, the give-up wording, and — deliberately — that the Auto-mode refusal from the backend is surfaced verbatim instead of being reworded into a generic failure, since the user needs to know to switch modes |
...
6. ~~9.0.9 verification tool and 9.0.8 write tool~~ **Done (2026-09-07)**: `workspace_run_command` gated by `allowCommandRun`, `workspace_write_file` gated by `auto` mode with every write published back as an undoable applied diff. Next in this block is 9.0.10: the orchestrator does not yet drive verify → repair → re-verify itself, so a failed stage still aborts the pipeline. 9.5.1 is no longer blocked on a missing write path.
...
*Last updated: 2026-09-07 - Phase 9.0: native tool calling (9.0.1), AGENTS.md memory (9.0.2), and the MCP client (9.0.3) are done. New 9.0.5-9.0.13 follow-up block: the context budget uses priority quotas instead of order-dependent greedy trimming, token estimation is CJK-aware, a resumed pipeline rebuilds tool definitions and the invoker together, and the Agent can now run the project's own checks (`workspace_run_command`, 9.0.9) and write files in `auto` mode (`workspace_write_file`, 9.0.8) with every write published back as an undoable applied diff. `allowCommandRun` and Agent mode are now real backend gates. The remaining gap in this block is 9.0.10: the orchestrator does not yet drive verify → repair → re-verify, so a failed stage still aborts the pipeline. Phase 10.1 CI covers Windows, Linux, and macOS; Phase 10.7 diff staleness detection is real and confirmed by a headless `agent_cli` end-to-end run.*
