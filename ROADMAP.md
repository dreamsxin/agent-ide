...
| 9.0.10 Autonomous Bounded Repair Loop | The prompt builder and check runner are already shared (`services/verification.rs`, `verify_workspace`, `agent_repair_prompt`), and `Verify All` / `Fix with Agent` send a repair prompt — but each is one user-triggered round. Missing: the orchestrator itself running verify → repair → re-verify within a bounded iteration count, so a failed stage or failed check does not abort the pipeline | Critical | **In progress**: the loop's decision rule is now shared and tested (`verification::RepairPolicy`) instead of living inside the CLI's loop body, and the CLI drives itself from it. Still to do: the orchestrator calling it, which needs a desktop equivalent of "apply, re-run checks, feed failures back" |
