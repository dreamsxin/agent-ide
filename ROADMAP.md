...
| 9.0.11 Persistent Run Message Thread | Replace prose concatenation of `prior_outputs` with a real message list so tool results survive across stages and prompt caching becomes possible | High | Planned. **Interim fix landed (2026-09-07)**: that concatenation had no size limit at all, and it bypasses the context budget (the budget only trims `context`), so a long pipeline grew the request without bound and pushed real project context out of the window. `executor::join_prior_outputs` keeps the two most recent stages verbatim and tail-truncates older ones with the omission stated. **Safety net landed (2026-09-07)**: prompt contract tests (`RequestRecorder`) pin what each stage's request must contain, so the restructure can be checked for missing pieces. Still not attempted, deliberately: the rewrite changes every stage's prompt structure, and judging whether the new structure produces *better* output needs a real provider and a human — no test here can measure that |
