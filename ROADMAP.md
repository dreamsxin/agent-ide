...
1. **Diff application still lacks version-aware hunks**
   ...
   - **`summary.json` reports `apply: null` even when the apply succeeded (found 2026-09-07, unfixed).**
 Verified twice against DeepSeek: `status` is `applied`, exit code 0, both files correctly rewritten on disk — and the `apply` field in the run summary is `null`, so the record shows no applied or failed list at all. `apply_pending_diffs` runs at `cli/mod.rs:936`, its result is pushed into `apply_results` at :946 and merged at :1048, so the value exists somewhere between there and the artifact write; the summary handed to `write_artifacts` does not carry it. Nothing in the automated suite noticed because the CLI apply tests assert on the workspace files and on `changes.json`, not on `summary.apply`. Worth fixing with a test that asserts the summary's own apply counts, since a run record that omits what it changed is the one artifact an autonomous agent has to trust.
