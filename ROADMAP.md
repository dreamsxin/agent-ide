...
1. **Diff application still lacks version-aware hunks**
   ...
   - **Models do not quote `original` verbatim (found 2026-09-07 by an `--apply` eval run, unfixed).** With `deepseek-v4-flash` on the legacy diff-block protocol, every hunk failed with `Could not find original content`, and the reason was not staleness: the model's quoted original said `` `Hello, ${name}!` `` where the file has `` `Hello, ${name}` ``, and `import { greet } from './greet';` with single quotes where the file uses double. It rewrote the context while quoting it. Strict matching is right to refuse — applying an edit whose surrounding context was misquoted is how you corrupt a file silently. So the fix is not fuzzier matching but making the model quote exactly: either require the structured agent-changes protocol for this provider instead of legacy blocks, or state the verbatim-quoting requirement in the diff instructions and re-measure. Needs a provider run to verify either way, which is what `scripts/eval/deepseek-pipeline.ps1 -Apply` is for.
