...
18. Wire `agent_cli smoke ide-surface` into CI. It needs no provider and no workspace fixture, so it is the cheapest available regression net for the panel backends — but it is not in `ci.yml` yet, so nothing runs it automatically.
