...
Goal: close the loop. The read side and the verify side now exist; writing is still the missing half, and it blocks 9.5.1 — parallel subagents that cannot write have nothing to isolate.
...
| 9.0.8 Workspace Write Tool | `workspace_write_file` behind `resolve_for_agent_write`, the path deny list, and `allowFileCreate` / `allowFileDelete`; every call checkpointed for undo and action-logged | Critical | Planned |
...
| Task | Deliverable | Priority | Status |
|------|-------------|----------|--------|
| 9.5.1 Parallel Subagents + Worktree Isolation | Upgrade the serial role pipeline to a DAG: coder steps can fan out in parallel, each in an isolated `git worktree`, with reviewer-stage merge and summary | Critical | Planned. **Blocked on 9.0.8/9.0.9**: agents that cannot write files have nothing to isolate |
...
6. ~~9.0.9 verification tool~~ **Done (2026-09-07)**: `workspace_run_command` gated by `allowCommandRun`. Next in this block is 9.0.8, the workspace write tool: the model can now read the workspace and run the project's checks, but it still cannot observe the result of its own change, and 9.5.1 stays blocked until it can.
...
*Last updated: 2026-09-07 - Phase 9.0: native tool calling (9.0.1), AGENTS.md memory (9.0.2), and the MCP client (9.0.3) are done. New 9.0.5-9.0.13 follow-up block: the context budget now uses priority quotas instead of order-dependent greedy trimming, token estimation is CJK-aware, a resumed pipeline rebuilds tool definitions and the invoker together, and the Agent can now run the project's own check commands through `workspace_run_command` (9.0.9) — which makes `allowCommandRun` the third permission with real backend enforcement. The remaining structural gap is the write tool (9.0.8), which also blocks 9.5.1. Phase 10.1 CI covers Windows, Linux, and macOS; Phase 10.7 diff staleness detection is real and confirmed by a headless `agent_cli` end-to-end run.*
