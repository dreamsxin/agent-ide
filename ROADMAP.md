...
16. **A run holds an exclusive lock on state other commands need (audited 2026-09-08)**
   - `send_agent_prompt` (`commands/agent.rs:346`) takes `Mutex<AgentOrchestrator>` and holds it across `orch.run(...).await` — the whole pipeline. Every other command touching the orchestrator queues behind it. `stop_agent` still works because it sets `cancel_flag` before locking (`agent.rs:549`), and undo availability no longer needs the lock because it rides in the `agent-state-changed` payload. What remains blocked: `get_agent_state`, `get_agent_diffs`, and the apply/reject family. The last group *should* block — applying while the pipeline rewrites `diffs` is not something to allow — so the practical remaining impact is the two mount-time queries.
   - **Do not fix this by splitting the orchestrator into per-field locks.** The audit found four invariants that span fields and are safe today only because each is one synchronous function: (a) `apply_diffs_to_fs` pushes the undo checkpoint at `orchestrator.rs:920` *before* flipping diff statuses at `:922-928`; (b) `record_tool_writes` pushes a checkpoint, extends `diffs`, then re-stamps **every** diff's `baseHash` (`:293-298`) — so `diffs` cannot be split per-diff either, and the upstream `permissions.take_writes()` drain (`agent.rs:450`) must be in the same critical section or writes exist on disk with no diff card and no undo entry; (c) pause snapshots `paused_run` + pipeline marks + state together (`:594-617`), and `continue_agent_pipeline` reads five fields as one consistent set (`agent.rs:824-853`); (d) `record_step_success`. Splitting locks means re-establishing all four by hand.
   - **The right shape is to stop holding the lock across the awaits, not to split it.** `run` has only four direct mutations (`orchestrator.rs:446, 448, 540, 544`); `continue_pipeline_from` has ~18, but all of them are synchronous — `self` is not borrowed across either await point (`execute_stage` at `:663`, `sleep` at `:786`). So `continue_pipeline_from` should take `&Arc<Mutex<AgentOrchestrator>>` instead of `&mut self` and re-lock briefly at each mutation site, exactly as `run_agent_step` already does at the step level (`agent.rs:706-729`, releases across the LLM call, reacquires at `:740`). Roughly 40 mechanical sites plus the orchestrator tests that call `run` / `continue_pipeline_from` directly. The four invariants survive unchanged because each stays inside one guard.
   - Two of those invariants are now pinned by tests and named in doc comments (`a_partly_failed_auto_apply_still_leaves_a_way_back`, plus the `apply_diffs_to_fs` / `record_tool_writes` headers), so the migration fails loudly instead of silently losing an undo entry.
