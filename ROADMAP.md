...
14. **CLI mode is useful but not feature-complete**
   ...
   - CLI now uses the shared project command runner for `--run-command`, records parsed backend Problems, and can feed failed command output into bounded repair iterations with `--max-iterations` guarded by `--allow-run`.
...
8. Continue shared backend refactor by moving Agent run artifacts behind reusable services used by both Tauri commands and CLI; add richer failure-chain artifacts, timeouts, and broader permission policy for CLI repair loops.
