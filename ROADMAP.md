...
5. Finish Phase 9.0.4 desktop permission model V2. Done so far: MCP tool approval (`McpToolPolicy` + per-server `autoApprove`), `allowFileCreate` for Auto-mode apply, and path deny rules for Agent writes, all enforced in `src-tauri`. Still open: a per-run cost cap and exposing MCP tools to the CLI behind the same model. `allowFileDelete` / `allowCommandRun` / `allowGitActions` need an Agent-reachable backend path before they can be enforced at all — see the Current State note.
...
*Last updated: 2026-09-07 - Phase 9.0: native tool calling (9.0.1), AGENTS.md memory (9.0.2), and the MCP client (9.0.3) are done; permission model V2 (9.0.4) enforces MCP tool approval and Auto-mode file creation backend-side, and MCP tool results are now size-capped with qualified-name collisions rejected instead of silently misrouted. Phase 10.1 CI covers Windows, Linux, and macOS; Phase 10.7 diff staleness detection is real and confirmed by a headless `agent_cli` end-to-end run.*
