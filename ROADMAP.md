...
Status as of 2026-09-08: **Phase 8 daily IDE replacement hardening in progress; Phase 9.0 market-parity foundation nearly closed.** Landed since the 09-05 snapshot: priority-quota context budget, CJK-aware token estimate, tool surface on resumed runs, the write and verification tools, CLI tool parity, the bounded repair loop with its UI, and the per-run monetary spend cap (9.0.13). Still open in 9.0: the persistent run message thread (9.0.11, interim size bound only).
...
| 9.0.11 Persistent Run Message Thread | Replace prose concatenation of `prior_outputs` with a real message list so tool results survive across stages and prompt caching becomes possible | High | Planned. **Interim fix landed (2026-09-07)**: that concatenation had no size limit at all, and it bypasses the context budget (the budget only trims `context`), so a long pipeline grew the request without bound and pushed real project context out of the window. `executor::join_prior_outputs` keeps the two most recent stages verbatim and tail-truncates older ones with the omission stated. **Safety net landed (2026-09-07)**: prompt contract tests (`RequestRecorder`) pin what each stage's request must contain, so the restructure can be checked for missing pieces. **Eval runner landed (2026-09-07)**: `scripts/eval/deepseek-pipeline.ps1` runs one real pipeline against DeepSeek in a throwaway workspace and leaves the artifacts for a human to judge — preview-only by default, so one eval costs one model call. Blocked on a DeepSeek API key: none is configured in this environment, so the runner has never actually been executed |
